<?php 
    ...
    class socialWork_gestion{
        ...
        public function addActivity($date_creation,$places,$id_type_activite,$id_demandeur,$description,$icon_activite,$nom){
            ...
            $stmt=$this->connexion->prepare($sql);
            $stmt->bindParam(1,urlencode($date_creation),PDO::PARAM_INT);
            $stmt->bindParam(2,urlencode($places),PDO::PARAM_INT);
            $stmt->bindParam(3,urlencode($id_type_activite),PDO::PARAM_INT);
            $stmt->bindParam(4,urlencode($id_demandeur),PDO::PARAM_INT);
            $stmt->bindParam(5,urlencode($description),PDO::PARAM_STR);
            $stmt->bindParam(6,urlencode($places_actuel),PDO::PARAM_INT);
            $stmt->bindParam(7,urlencode($icon_activite),PDO::PARAM_STR);
            $stmt->bindParam(8,urlencode($nom),PDO::PARAM_STR);
